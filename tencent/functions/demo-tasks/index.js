...
function mapTask(row) {
  return {
    ...
    recommended_mode: String(row.recommended_mode ?? 'mock'),
...
async function fetchDemoTasks() {
  const result = await db
    ...
    .select('id,title,description,prompt,category,recommended_mode,sort_order,is_enabled,metadata,created_at,updated_at')
    .eq('is_enabled', 1);

  assertNoQueryError(result);

  return extractRows(result)
    .filter((row) => normalizeBoolean(row.is_enabled))
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    const tasks = await fetchDemoTasks();
    sendJson(res, 200, {
      ok: true,
      data: {
        tasks,
    ...
    console.error('[demo-tasks] query failed', error && error.message ? error.message : error);
    sendError(res, 500, 'db_error', '读取示例任务失败。');
  }
});

server.listen(PORT, HOST, () => {
  console.log(`[demo-tasks] listening on ${HOST}:${PORT}`);
});
