...
所有 `JSON NOT NULL` 字段，后端 repository 插入时必须显式写入 `{}`、`[]` 或完整 JSON，不依赖数据库默认值。
...
2. 通过 `_openid` 查 `app_profiles`，得到业务 `user_id`。第一阶段 `_openid` 与 `user_id` 可以同值。
...
3. 新增 CloudBase Auth 后端校验 helper，建立 `_openid -> app_profiles.user_id` 映射。
4. 迁移 conversations/messages/report/run 查询接口，所有 SQL 显式加 `_openid/user_id`。
