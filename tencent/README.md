...
当前迁移状态见 `../docs/TENCENT_MIGRATION_STATUS.md`。本目录保留腾讯云单轨实现的迁移草案和 CloudBase MySQL schema；Tencent-09A 仅表示 CloudBase Auth helper 与正式 `/api/auth/me` 验证完成，Tencent-10C/Tencent-13 仅新增 conversations / messages / reports / demo-copy / quota 基础闭环函数，Tencent-15 在 Agent Run 函数中保留固定 `basic` 验证路径并新增 `real` planner / tools / Groq / fallback 验证路径，当前仓库仍不迁移前端 Auth store、不修改前端状态、不切换前端正式 Agent Run 调用。
...
- `functions/workbench-agent-run-stream/`：Tencent-15 Agent Run 流式验证 HTTP Function，固定路由为 `/api/agent/run/stream`，保留 `basic` mock 基础闭环，并新增 `real` planner、受控 data tools、Groq 和明确 fallback 路径。
...
6. 迁移 Agent Run SSE。Tencent-15 已在 CloudBase 函数内接入 planner、受控 data tools、Groq 和明确 fallback，用于本地测试面板验证；后续仍需补 quota 事务、RAG knowledge_qa、报告生成入口、真实前端切换和断线恢复验证。
