...
当前迁移状态见 `../docs/TENCENT_MIGRATION_STATUS.md`。本目录保留腾讯云单轨实现的迁移草案和 CloudBase MySQL schema；Tencent-09A 仅表示 CloudBase Auth helper 与正式 `/api/auth/me` 验证完成，Tencent-10C/Tencent-13 仅新增 conversations / messages / reports / demo-copy / quota 基础闭环函数，当前仓库仍不迁移前端 Auth store、不修改前端状态。
