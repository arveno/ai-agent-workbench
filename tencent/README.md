...
当前迁移状态见 `../docs/TENCENT_MIGRATION_STATUS.md`。本目录保留腾讯云单轨实现的迁移草案和 CloudBase MySQL schema；Tencent-09A 仅表示 CloudBase Auth helper 与正式 `/api/auth/me` 验证完成，Tencent-10C/Tencent-13 仅新增 conversations / messages / reports / demo-copy / quota 基础闭环函数，Tencent-14 仅新增固定 Agent Run 基础闭环验证，当前仓库仍不迁移前端 Auth store、不修改前端状态。
...
- `functions/workbench-agent-run-stream/`：Tencent-14 Agent Run 基础闭环 HTTP Function，固定路由为 `/api/agent/run/stream`，验证 Auth、会话归属、quota、SSE、`agent_runs`、`run_events`、mock `tool_invocations` 和 assistant message 写入。
...
Tencent-13 已新增 `workbench-quota` 基础闭环函数，用于读取本月额度、消耗一次额度并完成 `agent_run_usage`。当前实现暂不接真实 Agent Run，不接 SSE，也没有并发事务保护；接入真实 Agent Run 前，`consume_agent_run_quota` 必须升级为 MySQL 事务：
...
6. 迁移 Agent Run SSE。Tencent-14 先新增固定 Agent Run 基础闭环以验证 CloudBase HTTP Function 流式输出、quota 和 run 持久化，后续再接真实 planner/tools/Groq、报告生成入口和前端切换。
