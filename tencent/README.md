...
通过 CloudBase Node SDK 写入 MySQL `JSON` 字段时，必须先使用 `JSON.stringify(...)`，不能直接传 JS object / array。读取后再安全 `JSON.parse`，解析失败时按字段语义回退到 `{}` 或 `[]`。
...
2. 通过 `_shared/auth.js` 查或创建 `app_profiles`，得到业务 `user_id`。第一阶段 `_openid` 与 `user_id` 可以同值。
