...
当前迁移状态见 `../docs/TENCENT_MIGRATION_STATUS.md`。本目录保留腾讯云单轨实现的迁移草案和 CloudBase MySQL schema；Tencent-09A 仅表示 CloudBase Auth helper 与正式 `/api/auth/me` 验证完成，Tencent-10C 仅新增 conversations / messages 基础闭环函数，当前仓库仍不迁移前端 Auth store、不修改前端状态。
...
4. 迁移 conversations/messages/report/run 查询接口，所有 SQL 显式加 `_openid/user_id`。当前 Tencent-10C 只新增 conversations 列表 / 创建和 messages 读取 / 写入基础闭环，PATCH、archive、reports 和 run 查询后续再迁。
