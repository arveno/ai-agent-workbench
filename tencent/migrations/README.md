...
CloudBase RunSql 更适合单条或分段 SQL 执行，不建议一次性粘贴完整长 SQL。执行 `001_cloudbase_mysql_schema.sql` 时，建议每个 `CREATE TABLE ... ENGINE=InnoDB ...;` 语句单独执行。`002_cloudbase_teaching_metrics.sql` 独立创建公开演示数据源表，可在 `001` 全部完成后执行。
