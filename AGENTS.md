...
- 自动添加、移除或伪造 `review:approved` label；新 commit 后不得沿用旧审批 label。
- 处理 stash。
- 全项目格式化。
- 新增无关依赖。
- 修改任务范围外文件。
...
合并 PR 前，Codex 必须确认 `Review Gate` 已通过。若 `Review Gate` 未通过，或 `review:approved` 早于 PR 最新 head commit，必须停止，不得 merge。
