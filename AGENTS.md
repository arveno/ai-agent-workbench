...
- 手动添加、移除或伪造 `review:approved` / `review:stale` label；新 commit 后不得沿用旧审批 label。
- 处理 stash。
- 全项目格式化。
- 新增无关依赖。
- 修改任务范围外文件。
...
合并 PR 前，Codex 必须确认 `Review Gate` 已通过。若 `Review Gate` 未通过，必须停止，不得 merge。新 commit push 到 PR 后，旧 `review:approved` label 优先由 Review Gate 自动移除，并由 Review Gate 尝试添加 `review:stale`；如果自动处理失败，只能由用户手动处理并在重新 review 后手动添加 `review:approved`，Codex 不得手动操作。
