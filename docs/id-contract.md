...
本文档冻结 AI Agent Workbench 的核心对象与 ID 契约。后续涉及 Run、Trace、Report、Source、Usage、Evaluation 的代码修改，都必须先对齐本文。
...
当前最大风险是 `runId` / `clientRunId` / `runtimeRunId` 三轨混用：
...
1. canonical `runId` = DB `agent_runs.id`。
2. `clientRunId` = 前端请求幂等 / pending key，不作为主外键。
3. `runtimeRunId` = 兼容读取 / metadata，不作为 UI 主 ID，不作为主外键。
4. `currentRun.id` / `RunSnapshot.id` 后续应代表 canonical `runId`。
5. 如果需要展示短 ID，使用 `displayRunId`。
6. 旧数据可以短期按 `runtimeRunId` 兼容读取，但新数据不得继续把 `runtimeRunId` 当主关系。
7. 所有执行后资产必须尽量绑定 canonical `runId`。
8. 组件层不得自行判断 raw id 语义，ID 归一必须发生在 mapper / service / store 边界层。
...
| `clientRunId` | 前端创建 run 前生成的请求幂等 / pending key。 | 前端发起 run 时生成。 | 可以写入 `agent_runs.runtime_run_id` 或 metadata。 | 否 | 仅调试或 pending 状态可见。 | 是 | 可以 | 禁止作为 run_events / messages / reports / usage 的主外键。 |
| `runtimeRunId` | 迁移期兼容 ID，当前通常等于 `clientRunId`。 | 后端从 `clientRunId` 派生或从旧数据读取。 | 是，存在于 `agent_runs.runtime_run_id` 或 metadata。 | 否 | 不作为 UI 主 ID；可用作旧数据显示辅助。 | 可以用于旧请求幂等 | 是 | 禁止作为 `currentRun.id`、`RunSnapshot.id` 或新数据主关系。 |
...
- `runtimeRunId`：迁移期兼容字段，当前通常等于 `clientRunId`。
- `displayRunId`：UI 展示用，可以来自 `runId` 短格式或 `runtimeRunId`，但不能参与持久化关系。
...
- `metadata.runtimeRunId` 只用于旧数据兼容。
...
- `RunSnapshot` 显式拆分 `id` / `clientRunId` / `runtimeRunId` / `displayRunId`。
...
- `runtimeRunId` 独立字段存在，只用于兼容和 metadata。
...
`clientRunId` 解决前端发起请求时的 pending 和幂等问题，`runtimeRunId` 解决旧数据和迁移期兼容问题。

完成后，live run 与 refresh run 会使用同一套 ID 语义，后续 Evaluation / Bad Case / Improvement 才能建立可信的质量闭环。
