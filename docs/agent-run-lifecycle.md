...
- Report / Evaluation 同步模型状态时，只继承项目 canonical run metadata 中的 provider、model、latency、usage、costEstimate、fallback 和 model error 语义；不得重新解析 LangChain raw payload，LangSmith 外部 ID 不替代项目 Evaluation 主关系。
