...
当前状态表示 conversations 列表 / 创建、messages 读取 / 写入、reports 列表 / 单条读取 / 保存、demo-copy、quota 基础闭环函数和 Agent Run CloudBase 流式验证函数已加入仓库并可进行部署验证，其中前端 CloudBase preview 已接入 conversations、messages、reports、demo-copy、quota 和 Agent Run stream。`PATCH`、`DELETE`、archive、Agent Run 报告生成、RAG knowledge_qa、前端 `authStore` 和正式默认链路均未迁移。Tencent-10C 暂未在消息写入和会话计数更新之间使用事务；Tencent-13/Tencent-15 暂未在 quota consume 中使用 MySQL transaction + 行锁，后续高并发场景需要补事务或原子更新方案。
...
5. 最后迁 Agent Run SSE。Tencent-15 已在 CloudBase Agent Run 函数中接入 planner、受控 data tools、Groq 和明确 fallback；Tencent-17 已在 CloudBase preview 开关下接入前端 stream 调用。后续仍需补 RAG knowledge_qa、报告生成入口、事务化 quota、断线恢复和正式切换前回归测试。
